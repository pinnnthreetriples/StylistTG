...
def test_map_step_to_tdlib_query_for_profile_operations() -> None:
    ...
    assert (
        map_step_to_tdlib_query({"step_type": "set_bio", "payload": {"bio": "Hello"}})["@type"]
        == "setBio"
    )
    assert (
        map_step_to_tdlib_query({"step_type": "set_username", "payload": {"username": "stylist"}})[
            "@type"
        ]
        == "setUsername"
    )
...
def test_username_verify_policy_marks_ambiguous_result_uncertain() -> None:
    verification = verify_username_result(
        ...
        me_response={
            "usernames": {"editable_username": "other-user", "active_usernames": ["other-user"]}
        },
    )
...
class StoryPostClient:
    ...
    def __init__(
        self, *, can_post_response: dict | None = None, story_post_succeeds: bool = True
    ) -> None:
        self.closed = False
        self.queries: list[dict] = []
...
def test_profile_adapter_emits_hard_stop_error_code_for_frozen_tdlib_error(tmp_path) -> None:
    ...
    events = list(
        adapter.execute(
            ...
            {
                "steps": [
                    {"step_key": "set_name:0", "step_type": "set_name", "payload": {"name": "A"}}
                ]
            },
            {},
        )
