...
def test_story_draft_crud_contract(db_session) -> None:
    ...
    patch_response = client.patch(f"/api/story-drafts/{draft['id']}", json={"caption": "Updated"})
    assert patch_response.status_code == 200
    assert patch_response.json()["caption"] == "Updated"

    list_response = client.get(f"/api/story-drafts/{account.id}")
    assert list_response.status_code == 200
    assert [item["id"] for item in list_response.json()] == [draft["id"]]

    delete_response = client.delete(f"/api/story-drafts/{draft['id']}")
    assert delete_response.status_code == 204
    assert get_asset(db_session, asset.id).status == "orphaned"
    app.dependency_overrides.clear()


def test_story_draft_rejects_unchecked_premium_active_period(db_session) -> None:
    account = create_account(db_session, external_ref="primary")
    asset = seed_story_asset(db_session)
    db_session.commit()
