    ...
    *,
    ...
    try:
        ...
        return RateLimitDecision(
            allowed=False, reason="rate_limit_store_unavailable", retry_after_seconds=60
        )
    if count > limit:
        return RateLimitDecision(
            allowed=False,
            ...
            else "account_rate_limit_exceeded",
            retry_after_seconds=max(ttl, 1),
            remaining=0,
        )
    return RateLimitDecision(
        allowed=True, remaining=max(limit - count, 0), retry_after_seconds=max(ttl, 1)
    )
...
def _limit_for(action_type: str, *, queue_name: str | None, config: Settings) -> int:
    ...
    if (
        ...
        or action_type.startswith("profile.")
        or action_type == "job.enqueue"
    ):
...
def _key(
    *, workspace_id: str, action_type: str, account_id: str | None, queue_name: str | None
) -> str:
