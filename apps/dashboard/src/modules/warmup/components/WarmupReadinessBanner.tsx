  ...
  return (
    <Alert icon={readiness.dry_run ? <Info className="size-4" /> : <ShieldCheck className="size-4" />}>
      ...
      <div className="mt-3 grid gap-2 text-xs text-muted-foreground sm:grid-cols-3">
        <span className="rounded-md bg-card/70 px-2.5 py-1.5">1. Проверка готовности</span>
        <span className="rounded-md bg-card/70 px-2.5 py-1.5">2. План по выбранному пресету</span>
        <span className="rounded-md bg-card/70 px-2.5 py-1.5">{readiness.dry_run ? '3. Аудит без live-действий' : '3. Live-выполнение по расписанию'}</span>
      </div>
    </Alert>
  )
}
