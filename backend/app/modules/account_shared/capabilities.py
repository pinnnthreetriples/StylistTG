"""Account capability composition shared across feature modules.

This module owns the canonical implementation of
`build_account_capabilities` so that `account_shared` does not depend on
`account_core`. `account_core.capabilities` re-exports from here for
backwards compatibility, and any feature module that needs to compose
capability read models depends on `app.modules.account_shared.interfaces`.
"""
