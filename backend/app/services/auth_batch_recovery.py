...
def recover_auth_batches(session: Session) -> int:
    ...
    running_batches = list(
        session.execute(select(AuthBatch).where(AuthBatch.status == AuthBatchStatus.RUNNING)).scalars()
    )
...
def release_expired_locks(session: Session) -> int:
    ...
    for item in items:
        ...
        if item.attempt_count < MAX_ATTEMPTS:
            transition_item(item, AuthBatchItemStatus.QUEUED, actor="system", payload={"reason": "lock_expired"})
        else:
            transition_item(item, AuthBatchItemStatus.TIMED_OUT, actor="system", payload={"reason": "lock_expired"})
    session.commit()
    return len(items)
...
def expire_waiting_inputs(session: Session, *, timeout_seconds: int = 180) -> int:
    ...
    items = list(
        session.execute(
            select(AuthBatchItem)
            .where(AuthBatchItem.status.in_([AuthBatchItemStatus.WAITING_CODE, AuthBatchItemStatus.WAITING_2FA]))
            .where(AuthBatchItem.updated_at < cutoff)
        ).scalars()
    )
    for item in items:
        transition_item(item, AuthBatchItemStatus.TIMED_OUT, actor="system", payload={"reason": "input_timeout"})
    session.commit()
    return len(items)
