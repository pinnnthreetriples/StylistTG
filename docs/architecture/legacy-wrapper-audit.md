...
| `app.api.neuro_commenting` | `app.modules.neuro_commenting.router` | Preserve public import path for `/api/neuro-commenting`. | `sys`, canonical router module. | New route logic, auth changes, response mapping. | Remove only after router import audit shows no `app.api.neuro_commenting` users. | Neuro-commenting runtime API tests and compatibility monkeypatch call-sites. |
