...
- `<backend/app/modules/warmup/bootstrap_pool/>` owns the curated public-channel bootstrap pool. If a strategy/snapshot has no target channels, dispatch resolves safe active bootstrap targets before calling `channel_state.selector.choose_actions`; selector still remains the only action/target decision point. Bootstrap health checks are local/no-TDLib unless an operator explicitly approves live validation.
- `<backend/app/adapters/ai_profile_provider/>` owns profile bio/avatar AI providers. Default provider is deterministic `fake`; OpenAI/Anthropic require explicit live enablement plus SecretStr API key. `<backend/app/modules/account_editing/ai_generation.py>` owns uniqueness retry (`max_attempts=5` by default), workspace/account daily limits, sanitized operation logs, and preview-only generation; generated content is not applied to Telegram/profile state until a separate explicit account update.
