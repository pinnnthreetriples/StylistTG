...
class AccountOnboardingBatch(Base):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "idempotency_key", name="uq_account_onboarding_batch_workspace_idempotency"),
    ...
    consent_actor_user_id: Mapped[str | None] = mapped_column(UUIDString, ForeignKey("app_user.id"), nullable=True)
    ...
    items: Mapped[list[AccountOnboardingItem]] = relationship(back_populates="batch", cascade="all, delete-orphan", order_by="AccountOnboardingItem.position")
    artifacts: Mapped[list[AccountOnboardingArtifact]] = relationship(back_populates="batch", cascade="all, delete-orphan")
    events: Mapped[list[AccountOnboardingEvent]] = relationship(back_populates="batch", cascade="all, delete-orphan")
...
class AccountOnboardingItem(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_account_onboarding_item_workspace_phone_hash", "workspace_id", "phone_normalized_hash"),
    )

    id: Mapped[str] = mapped_column(UUIDString, primary_key=True, default=new_id)
    workspace_id: Mapped[str] = mapped_column(UUIDString, ForeignKey("workspace.id"), nullable=False)
    batch_id: Mapped[str] = mapped_column(UUIDString, ForeignKey("account_onboarding_batch.id"), nullable=False)
    account_id: Mapped[str | None] = mapped_column(UUIDString, ForeignKey("account.id"), nullable=True)
    auth_session_id: Mapped[str | None] = mapped_column(UUIDString, ForeignKey("telegram_auth_session.id"), nullable=True)
    artifact_id: Mapped[str | None] = mapped_column(UUIDString, ForeignKey("account_onboarding_artifact.id"), nullable=True)
...
class AccountOnboardingArtifact(Base):
    ...
    id: Mapped[str] = mapped_column(UUIDString, primary_key=True, default=new_id)
    workspace_id: Mapped[str] = mapped_column(UUIDString, ForeignKey("workspace.id"), nullable=False)
    batch_id: Mapped[str | None] = mapped_column(UUIDString, ForeignKey("account_onboarding_batch.id"), nullable=True)
    item_id: Mapped[str | None] = mapped_column(UUIDString, ForeignKey("account_onboarding_item.id"), nullable=True)
    source_type: Mapped[str] = mapped_column(String(64), nullable=False)
