...
`NEURO_COMMENT_AI_PROVIDER=fake` is staging-only. Set
`NEURO_COMMENT_AI_PROVIDER=openai_compatible`, `NEURO_COMMENT_AI_BASE_URL`, and
`NEURO_COMMENT_AI_API_KEY` before production or real neuro-commenting smoke.
