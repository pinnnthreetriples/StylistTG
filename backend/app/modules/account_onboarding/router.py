...
@router.post(
    ...
    response_model=AccountOnboardingSnapshotRead,
    status_code=status.HTTP_201_CREATED,
)
def create_account_onboarding_batch(
    payload: AccountOnboardingBatchCreate,
    session: Session = Depends(get_session),
    auth: AuthContext = Depends(require_mutation_permission),
):
    try:
        response, _created = service.create_batch(
            session, workspace_id=auth.workspace_id, user_id=auth.user_id, payload=payload
        )
        return response
...
def list_account_onboarding_batches(
    session: Session = Depends(get_session), auth: AuthContext = Depends(require_authenticated)
):
    return service.list_batches(session, workspace_id=auth.workspace_id)


@router.get(
    "/api/account-onboarding-batches/{batch_id}", response_model=AccountOnboardingSnapshotRead
)
def get_account_onboarding_batch(
    batch_id: UuidString,
    session: Session = Depends(get_session),
...
def validate_account_onboarding_batch(
    batch_id: UuidString,
...
def confirm_account_onboarding_batch(
    batch_id: UuidString,
...
def cancel_account_onboarding_batch(
    batch_id: UuidString,
...
@router.post(
    ...
    response_model=AccountOnboardingItemRead,
)
def retry_account_onboarding_item(
    batch_id: UuidString,
    item_id: UuidString,
    payload: AccountOnboardingMutationRequest,
    session: Session = Depends(get_session),
...
def submit_account_onboarding_code(
    batch_id: UuidString,
    item_id: UuidString,
...
def submit_account_onboarding_password(
    batch_id: UuidString,
    item_id: UuidString,
    ...
    try:
        return service.submit_password(
            session,
            workspace_id=auth.workspace_id,
            user_id=auth.user_id,
            batch_id=batch_id,
...
@router.post(
    ...
    response_model=AccountOnboardingArtifactRead,
    status_code=status.HTTP_201_CREATED,
)
def upload_account_onboarding_artifact(
    payload: AccountOnboardingArtifactCreate,
    session: Session = Depends(get_session),
    auth: AuthContext = Depends(require_mutation_permission),
):
    try:
        response, _created = service.upload_artifact(
            session, workspace_id=auth.workspace_id, user_id=auth.user_id, payload=payload
        )
        return response
    except OnboardingError as exc:
