...
def _check_app_env(report: CheckReport, env: dict[str, str] | None) -> None:
    ...
    if app_env not in {"staging", "production"}:
        report.add(
            "app_env",
            "WARN",
            "Cloud contour usually uses APP_ENV=staging or production",
            app_env=app_env,
        )
    else:
...
def _check_auth_mode(report: CheckReport, env: dict[str, str] | None) -> None:
    ...
    if is_cloud_env(env) and auth_mode == "local" and not allow_local:
        report.add(
            "auth_mode",
            "FAIL",
            "AUTH_MODE=local is not allowed for cloud mode",
            auth_mode=auth_mode,
        )
        ...
        report.add(
            "auth_mode",
            "WARN",
            "Cloud contour should use AUTH_MODE=supabase_jwt",
            auth_mode=auth_mode,
        )
    else:
...
def _check_operator_api(report: CheckReport, env: dict[str, str] | None) -> None:
    ...
    if is_cloud_env(env) and enforce_localhost:
        ...
        report.add(
            "operator_guard", "PASS", "Operator localhost guard is compatible with this contour"
        )
    ...
    if is_cloud_env(env) and api_stale_reaper:
        report.add(
            "api_stale_job_reaper", "FAIL", "Cloud API must set STALE_JOB_REAPER_ENABLED=false"
        )
    else:
        report.add(
            "api_stale_job_reaper", "PASS", "API stale job reaper is compatible with this contour"
        )
...
def _check_database_urls(report: CheckReport, env: dict[str, str] | None) -> None:
    ...
    if not runtime_url:
        report.add(
            "database_runtime_url", "FAIL", "DATABASE_RUNTIME_URL or DATABASE_URL is required"
        )
    else:
        ...
        if db_mode == "neon" and "-pooler" not in host:
            report.add(
                ...
                "WARN",
                "Neon runtime URL usually uses pooled host",
                url=sanitized_url(runtime_url),
            )
        else:
            report.add(
                "database_runtime_url",
    ...
    if not direct_url:
        ...
        if "-pooler" in host:
            report.add(
                "database_direct_url",
                "FAIL",
                "DATABASE_DIRECT_URL must not use Neon pooler host",
                url=sanitized_url(direct_url),
            )
        else:
            report.add(
                "database_direct_url",
                "PASS",
                "Direct migration database URL present",
                url=sanitized_url(direct_url),
            )
    if (
        runtime_url
        and direct_url
        and runtime_url == direct_url
        and not bool_env("ALLOW_IDENTICAL_DATABASE_URLS", env)
    ):
        report.add(
            "database_url_split",
            "FAIL",
            "Runtime and direct DB URLs must be separate unless explicitly allowed",
        )
    ...
    if db_mode != "neon":
        report.add(
            "db_connection_mode",
            "WARN",
            "Cloud contour should use DB_CONNECTION_MODE=neon",
...
def _check_storage(report: CheckReport, env: dict[str, str] | None) -> None:
    ...
    if storage_backend != "s3":
        report.add(
            "storage_backend",
            "FAIL",
            "Cloud object storage requires STORAGE_BACKEND=s3",
    ...
    if ttl < 60 or ttl > 3600:
        report.add(
            "signed_url_ttl", "FAIL", "Signed URL TTL must be between 60 and 3600 seconds", ttl=ttl
        )
    else:
    ...
    if looks_production(bucket) and app_env != "production":
        report.add(
            "storage_bucket_name",
            "FAIL",
            "Production-looking bucket name is not allowed outside production",
            bucket=bucket,
        )
...
def _check_tdlib(report: CheckReport, env: dict[str, str] | None) -> None:
    ...
    if live_enabled and not allow_live_smoke:
        report.add(
            "tdlib_live_enabled",
            "FAIL",
            "TDLib live mode must stay disabled for cloud smoke without explicit live approval",
        )
    else:
        report.add(
            "tdlib_live_enabled",
            "PASS",
            "TDLib live mode is safe for cloud smoke",
    ...
    if adapter == "tdlib" and not allow_live_smoke:
        report.add(
            "tdlib_adapter",
            "FAIL",
            "TDLib live adapter selected; refuse cloud smoke without explicit live approval",
        )
    else:
        report.add(
            "tdlib_adapter",
...
def _check_tdlib_root(
    report: CheckReport, tdlib_root: str | None, env: dict[str, str] | None
) -> None:
    if not tdlib_root:
        return
    asset_root = (
        env_value("STORAGE_LOCAL_ROOT", env) or env_value("LOCAL_STORAGE_PATH", env) or "storage"
    )
    ...
    if tdlib_path == asset_path or asset_path in tdlib_path.parents:
        report.add(
            "tdlib_storage_boundary",
            "FAIL",
            "TDLib root must not live under public asset storage",
            tdlib_root=tdlib_root,
        )
...
def _check_proxy_credentials_key(report: CheckReport, env: dict[str, str] | None) -> None:
    ...
    if not key:
        if is_cloud_env(env):
            report.add(
                "proxy_credentials_encryption_key",
                "FAIL",
                "Cloud proxy credentials require a Fernet encryption key",
            )
        else:
            report.add(
                "proxy_credentials_encryption_key",
                "WARN",
                "Proxy password encryption key is not configured",
            )
        return
    try:
        ...
        report.add(
            "proxy_credentials_encryption_key",
            "FAIL",
            "Proxy credentials encryption key is not a valid Fernet key",
        )
        return
    report.add(
        "proxy_credentials_encryption_key", "PASS", "Proxy credentials encryption key is valid"
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate StylistTG cloud dev/staging environment."
    )
