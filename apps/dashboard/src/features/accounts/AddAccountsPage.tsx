  ...
  async function run<T>(action: () => Promise<T>, quiet = false): Promise<T | null> {
    ...
    try {
      ...
      if (isSnapshot(result)) {
        setSnapshot(result)
        if (isTerminalOnboardingBatchStatus(result.batch.status)) {
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
      ...
      <section className="border-b border-border py-4">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Button disabled={!canCreate || busy} onClick={() => void createPreview()}>{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ArrowRight className="mr-2 h-4 w-4" />}Предпросмотр</Button>
          <Button disabled={busy} onClick={clearDraft} type="button" variant="secondary"><XCircle className="mr-2 h-4 w-4" />Очистить черновик</Button>
        </div>
      </section>
  ...
  return (
    <section className="rounded-lg border border-border bg-background p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div><div className="text-sm font-semibold text-foreground">3. Предпросмотр / выполнение</div><div className="text-xs text-muted-foreground">{onboardingStatusLabel(snapshot.batch.status)} · готово {snapshot.batch.counters.ready_count}/{snapshot.batch.counters.total_count}</div></div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] border-collapse text-sm">
          ...
          <tbody>{snapshot.items.map((item) => <tr className="border-b border-border/60" key={item.id}><td className="py-2">{item.phone_hint ?? item.username_hint ?? item.telegram_user_id_hint ?? item.label ?? 'Метаданные'}</td><td>{onboardingStatusLabel(item.status)}</td><td>{onboardingSourceLabel(item.source_type)}</td><td>{onboardingRiskLabel(item.risk_level)}</td><td>{itemMessage(item)}</td><td><ItemAction item={item} batchId={snapshot.batch.id} onAction={updateItem} /></td></tr>)}</tbody>
        </table>
      </div>
    </section>
  )
...
function ItemAction({ item, batchId, onAction }: { item: AccountOnboardingItem; batchId: string; onAction: (item: AccountOnboardingItem) => void }) {
  ...
  if (item.next_action === 'submit_code') {
    return <InlineSubmit label="Код Telegram" value={value} setValue={setValue} onSubmit={() => void submitAccountOnboardingCode(dashboardApiClient, batchId, item.id, { idempotency_key: makeOnboardingKey('code'), code: value }).then(onAction)} />
  }
  if (item.next_action === 'submit_password') {
    return <InlineSubmit label="Пароль 2FA" value={value} setValue={setValue} onSubmit={() => void submitAccountOnboardingPassword(dashboardApiClient, batchId, item.id, { idempotency_key: makeOnboardingKey('password'), password: value }).then(onAction)} />
  }
